...
. To view details for a latency event, select the event in the *Severity* column of the latency events table. This opens a latency analysis panel for that event.
...
. In the *Severity* column of the latency events table, select a latency event to open an analysis panel for that event.
+
...
. In the analysis panel, review the basic analysis results to understand the latency source.
. If the latency source is identified as data or cluster, select *Analyze*.
