...
* A bell indicator appears on the affected cluster in the clusters table.
* A summary card on the overview dashboard shows the total number of clusters with active recommendations.

Select a cluster with an active recommendation to view the recommendation card, which explains the suggested change and the reasoning, and a dry run section that shows the projected impact before you apply any changes.
