...
Latency analysis collects CloudWatch metrics for read and write operations on all FSx for ONTAP volumes linked to your AWS credentials. It continually checks these metrics against set thresholds to spot performance issues early.

When latency increases, Workload Factory automatically analyzes ONTAP QoS delay center metrics to find the main source of the slowdown. For more complex cases involving data or cluster components, you can optionally run AI analysis to receive a detailed root cause, a list of affected clients, and recommended steps to fix the issue.

=== Alert generation

An alert is triggered only if two conditions are met across the entire selected time range: latency is above its threshold and IOPS is above its threshold. Requiring both conditions helps prevent false alarms by confirming that high latency occurs while the system is under real load.
...
All detected events appear in the latency events table. If notifications are set up, you also receive an email or Amazon SNS message with details about the affected volumes.
...
NOTE: Basic analysis is available for all latency events when a link is associated with the FSx for ONTAP file system. If no link is associated, events can still be detected, but the analysis will provide limited information.
